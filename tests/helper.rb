...
def create_test_disk(connection, zone)
  ...
  disk = connection.disks.create(                                   :name => "fog-test-disk-#{random_string}",
                                   :size_gb => "10",
                                   :zone => zone,
...
def create_test_http_health_check(connection)
  random_string = SecureRandom.hex
  health_check = connection.http_health_checks.create(                                                        :name => "fog-test-check-#{random_string}")
  health_check
end
...
def create_test_backend_service(connection)
  ...
  backend_service = connection.backend_services.create(                                                         :name => "fog-test-backend-service-#{random_string}",
...
def create_test_url_map(connection)
  ...
  url_map = connection.url_maps.create(                                         :name => "fog-test-url-map-#{random_string}",
...
def create_test_target_http_proxy(connection)
  ...
  proxy = connection.target_http_proxies.create(                                                  :name => "fog-test-target-http-proxy-#{random_string}",
                                                  :url_map => url_map.self_link)
end

def create_test_zone_view(connection, zone)
  random_string = SecureRandom.hex
  zone_view = connection.zone_views.create(                                             :name => "fog-test-zone-view-#{random_string}",
                                             :zone => zone)
...
def create_test_target_pool(connection, region)
  ...
  target_pool = connection.target_pools.create(                                                 :name => "fog-test-target-pool-#{random_string}",
                                                 :region => region,
